...
If you don’t call the `start()` method Flowtime.js starts itself but some configuration parameters will be applied only after the first navigation action.
...
Sets the default values for parallax elements so you don’t have to set the values for every single element.
